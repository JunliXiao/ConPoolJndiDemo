...
//			System.out.println("Successfully connected to database!");
	...
	public User getById(int id) {
		User user = null;
		String sql = "select user_id, username, password, email, first_name, last_name, area, job, created_time from user where user_id = ?";
...
//				int userId = rs.getInt(1);
				...
				String job = rs.getString(8);
				String createdAt = rs.getString(9);
				user = new User(id, username, password, email, firstName, lastName, area, job, createdAt);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
